  ...
  const simulateTyping = useCallback((fullText, onComplete) => {
    ...
    const typeNextChar = () => {
      if (currentIndex < fullText.length) {
        ...
        let delay = 5; // Faster default speed

        if (char === "." || char === "!" || char === "?") {
          delay = 100; // Shorter pause at sentence endings
        } else if (char === "," || char === ";") {
          delay = 50; // Shorter pause at commas
        } else if (char === " ") {
          delay = 10; // Quick for spaces
        } else if (char === "\n") {
    ...
    responseTimeoutRef.current = setTimeout(typeNextChar, 10);
  }, []);
    ...
    async (text) => {
      ...
      try {
        ...
        simulateTyping(
          ...
          response.answer ||
            "🤖 Sorry, I couldn't find an answer. I'm in the learning stage and will improve over time.",
          (finalText) => {
            setMessages((prev) => [
              ...prev,
              {
  ...
  if (!mode) {
    return (
      <div
        ...
        } overflow-hidden z-[9999] bg-gradient-to-br from-white via-gray-50 to-green-50 ${
      ...
      >
        ...
        >
          ...
          <motion.div
            ...
            animate={{ y: 0, opacity: 1 }}
            transition={{ duration: 0.6 }} // 👈 delay removed
            whileHover={{
              scale: 1.02,
              y: -4,
              boxShadow:
          ...
          >
            ...
            <div className="flex items-center space-x-4 relative z-10">
              ...
              <div>
                <span
                  className={`text-gray-800 font-bold ${styles.textSize} group-hover:text-gray-900 transition-colors duration-100`}
                >
                  Need AI Assistant
                </span>
                <p className="text-gray-500 text-sm mt-1 group-hover:text-gray-600 transition-colors duration-100">
                  Get instant AI-powered help
                </p>
              </div>
            </div>
            <div className="text-gray-400 text-2xl group-hover:text-green-600 group-hover:translate-x-1 transition-all duration-100 relative z-10">
              ›
            </div>
          </motion.div>
          ...
          >
            <div className="absolute inset-0 bg-gradient-to-r from-blue-50/0 to-blue-100/20 opacity-0 group-hover:opacity-100 transition-opacity duration-100"></div>

            <div className="flex items-center space-x-4 relative z-10">
              <div
                className={`bg-gradient-to-br from-blue-100 to-blue-200 ${styles.cardPadding} rounded-2xl shadow-inner border border-blue-200/50 group-hover:shadow-md transition-all duration-100`}
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className={`${styles.iconSize} text-blue-600 group-hover:text-blue-700 transition-colors duration-100`}
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth={2}
            ...
            <div className="text-gray-400 text-2xl group-hover:text-blue-600 group-hover:translate-x-1 transition-all duration-100 relative z-10">
              ›
            </div>
          </motion.div>
        </div>
        ...
        >
          <div className="flex border-t border-gray-200/30 bg-white/50 backdrop-blur-sm">
            ...
            >
              ...
              <span
                className={`${styles.subtitleSize} font-semibold transition-colors duration-300`}
              >
                Home
              </span>
              <motion.div
          ...
          <div className="text-center mt-4 px-4">
            <div className="inline-flex items-center space-x-2 rounded-full px-4 py-2 ">
              ...
              <span
                className={`${styles.subtitleSize} text-[#085056] font-medium`}
              >
                Powered by Jaimax
              </span>
            </div>
  ...
  return (
    <motion.div
      ...
      className={`${styles.containerClass} ${
        styles.shadow
      } z-[9999] bg-gradient-to-br from-white via-gray-50 to-white flex flex-col overflow-hidden font-sans rounded-2xl backdrop-blur-sm ${
        styles.fullScreen ? "min-h-screen" : "h-[400px]"
      }`}
      ...
      >
        ...
        <div className="relative z-10">
          <motion.button
            ref={toggleRef}
            onClick={toggleMenu}
            className="h-10 w-10 flex items-center justify-center rounded-xl hover:bg-white/15 transition-all duration-200 text-white group relative overflow-hidden"
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
          >
            <motion.div
        ...
        <motion.button
          ...
          animate={
            isSending
              ? {
                  boxShadow: [
                    "0 0 0 0 rgba(8, 80, 86, 0.4)",
                    "0 0 0 10px rgba(8, 80, 86, 0)",
                  ],
                }
              : {}
          }
