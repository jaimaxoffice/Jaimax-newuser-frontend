...
//                           name="amount"
//                           autoComplete="off"
//                           className={`w-full px-2.5 py-1.5 bg-white text-sm border rounded focus:ring-2 focus:ring-[#1d8e85] focus:border-[#1d8e85] ${errors.amount
...
//       case "holdedInr":
//         return userData.data.holdedInr || 0;
//       case "p2pInr":
...
//   const getBalanceTypeName = () => {
//     switch (formData.balanceType) {
//       case "referral":
//         return "Available Balance";
//       case "holdedInr":
//         return "Holded Balance";
//       case "p2pInr":
//         return "P2P Balance";
//       case "wpStakingInr":
...
//       value: userData?.data?.Inr || 0,
//       color: "from-teal-500 to-teal-600",
//     },
//     {
//       type: "holdedInr",
//       label: "Holded Balance",
//       value: userData?.data?.holdedInr || 0,
//       color: "from-teal-500 to-teal-600",
//     },
//     {
//       type: "p2pInr",
//       label: "P2P Balance",
//       value: userData?.data?.p2pInr || 0,
//       color: "from-teal-500 to-teal-600",
//     },
//     {
//       type: "wpStakingInr",
//       label: "WP Staking Balance",
...
//     } else if (isNaN(amount) || parseFloat(amount) <= 0) {
//       errors.amount = "Amount must be a positive number";
//     } else if (inrBalanceTypes.includes(balanceType)) {
//       const settings = getSetting?.data;
//       if (settings) {
//         const currency =
//           paymentCurrency ||
...
//           errors.amount = `Insufficient balance. Available: ₹${availableBalance.toFixed(2)}`;
//         }
//       }
//     }
...
//     }
//   }, [formData.amount, formData.paymentCurrency, formData.balanceType, calculatePreview, userData?.data?.countryCode]);
...
//       paymentCurrency:
//         newBalanceType === "referral" ||
//         newBalanceType === "holdedInr" ||
//         newBalanceType === "p2pInr" ||
//         newBalanceType === "wpStakingInr"
//           ? userData?.data?.countryCode === 91
//             ? "INR"
//             : "USD"
//           : "",
...
//                   <div className="flex items-center justify-between">
//                     <div>
//                       <p className="text-xs opacity-90">Selected: {getBalanceTypeName()}</p>
//                       <p className="text-lg font-bold">
...
//                     </p>
//                   )}
//                   {inrBalanceTypes.includes(formData.balanceType) && (
//                     <p className="text-gray-600 mt-1 text-xs">
//                       Total Available: ₹{availableBalance.toFixed(2)}
//                     </p>
//                   )}
//                 </div>
...
const Withdrawal = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    paymentCurrency: "",
    amount: "",
    sourceOfWithdrwal: "Inr", // Added source of withdrawal
  });
  const [errors, setErrors] = useState({});
  ...
  const validate = () => {
    const errors = {};
    const { balanceType, paymentCurrency, amount, sourceOfWithdrwal } = formData;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { amount, balanceType, paymentCurrency, sourceOfWithdrwal } = formData;
      const currency =
        paymentCurrency ||
        (userData?.data?.countryCode === 91 ? "INR" : "USD");
      ...
      if (inrBalanceTypes.includes(balanceType)) {
        requestData = {
          ...
          currencyType: currency,
          balanceType: balanceType,
          sourceOfWithdrwal: sourceOfWithdrwal, // Include source of withdrawal
        };
      } else if (balanceType === "JAIMAX") {
        requestData = {
          amount: parseFloat(amount),
          currency: balanceType,
          sourceOfWithdrwal: sourceOfWithdrwal,
        };
      } else {
        toast.error("Invalid balance type");
        return;
  ...
  const onChangeBalanceType = (e) => {
    ...
    setFormData({
      amount: "",
      balanceType: newBalanceType,
      paymentCurrency: inrBalanceTypes.includes(newBalanceType)
        ? userData?.data?.countryCode === 91
          ? "INR"
          : "USD"
        : "",
      sourceOfWithdrwal: inrBalanceTypes.includes(newBalanceType) ? newBalanceType : "Inr",
    });
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full text-white px-2.5 py-1.5 text-sm border border-gray-300 rounded focus:ring-2 focus:ring-[#1d8e85] focus:border-[#1d8e85] bg-[#1d8e85]"
                  >
                    <option value="Inr">Available Balance</option>
                    <option value="holdedInr">Holded Balance</option>
                    <option value="p2pInr">P2P Balance</option>
                    <option value="wpStakingInr">WP Staking Balance</option>
                    <option value="JAIMAX">Purchase Token (JaiMax)</option>
