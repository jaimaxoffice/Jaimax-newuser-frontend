...
//   // Fixed: Calculate pagination properly
//   const totalPages = Math.max(1, Math.ceil(totalFilteredUsers / state.perPage));
//   const startIndex = (state.currentPage - 1) * state.perPage;
//   const paginatedData = filteredData.slice(startIndex, startIndex + state.perPage);

//   // Handle PerChange - Fixed to ensure currentPage stays within bounds
//   const handlePageChange = (e) => {
//     setLoading(true);
//     const newPage = Math.min(e, totalPages);
...
//   // Fix: Ensure currentPage is within bounds when data changes
//   useEffect(() => {
...
//   return (
//     <div className="min-h-screen bg-[#1d8d84] p-2 sm:p-4 lg:p-6 xl:p-8">
//       {/* Stats Cards - Fully responsive grid */}
//       <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3 sm:gap-4 lg:gap-6 mb-4 sm:mb-6 lg:mb-8">
//         {infoBoxes.map((box, idx) => (
//           <div
//             key={idx}
//             className="group relative overflow-hidden rounded-xl sm:rounded-2xl lg:rounded-3xl bg-white/80 backdrop-blur-sm p-3 sm:p-4 lg:p-6 shadow-lg border border-white/50 hover:shadow-2xl hover:scale-[1.02] transition-all duration-500 cursor-pointer"
//           >
//             {/* Background gradient */}
//             <div className={`absolute inset-0 bg-gradient-to-br ${box.gradient} opacity-5 group-hover:opacity-10 transition-opacity duration-500`}></div>
            
//             {/* Floating elements */}
//             <div className="absolute -top-2 -right-2 sm:-top-4 sm:-right-4 w-12 h-12 sm:w-16 sm:h-16 lg:w-24 lg:h-24 rounded-full bg-gradient-to-br from-white/20 to-transparent blur-xl group-hover:scale-110 transition-transform duration-500"></div>
...
//                   <div className="flex items-center justify-between flex-wrap gap-2">
//                     <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
//                       <div className={`p-1.5 sm:p-2 lg:p-3 rounded-lg sm:rounded-xl bg-gradient-to-br ${box.gradient} text-white shadow-lg group-hover:shadow-xl transition-shadow duration-300`}>
//                         {box.icon}
//                       </div>
//                       <div className="min-w-0">
//                         <h3 className="font-semibold text-gray-800 text-xs sm:text-sm lg:text-base truncate">
//                           {box.title}
//                         </h3>
//                         <p className="text-xs sm:text-sm text-gray-600 truncate">
//                           {box.description}
...
//                       <button
//                         onClick={handleCopyReferralCode}
//                         className={`p-1.5 sm:p-2 rounded-full bg-gradient-to-br ${box.gradient} text-white hover:shadow-lg transition-all duration-200 ${copiedCode ? 'scale-110' : ''}`}
//                         title="Copy referral code"
//                       >
//                         <Copy className="w-3 h-3 sm:w-4 sm:h-4" />
//                       </button>
//                       <button
//                         onClick={() => setShowReferralModal(true)}
//                         className={`p-1.5 sm:p-2 rounded-full bg-gradient-to-br ${box.gradient} text-white hover:shadow-lg transition-all duration-200`}
//                         title="Share referral"
//                       >
//                         <Share2 className="w-3 h-3 sm:w-4 sm:h-4" />
//                       </button>
//                     </div>
//                   </div>
//                   <div className="bg-gradient-to-r from-teal-50 to-white rounded-lg sm:rounded-xl p-2 sm:p-3 lg:p-4 border border-teal-200">
//                     <div className="flex items-center justify-between gap-2">
//                       <code className="text-sm sm:text-lg lg:text-xl font-bold text-teal-800 truncate flex-1">
//                         {box.value}
//                       </code>
//                       <span className="text-xs text-teal-600 whitespace-nowrap">
//                         {copiedCode ? '✓ Copied!' : 'Click to copy'}
...
//                     )}
//                   </div>
//                   <div className="text-lg sm:text-2xl lg:text-3xl font-bold text-gray-800">
//                     {typeof box.value === 'number' ? box.value.toLocaleString() : box.value}
//                   </div>
//                 </div>
//               )}
...
//       {/* Team Data Section - Enhanced responsive design */}
//       <div className="bg-white/80 backdrop-blur-sm rounded-xl sm:rounded-2xl lg:rounded-3xl shadow-xl border border-white/50 p-3 sm:p-4 lg:p-6 xl:p-8">
//         {/* Header with Search and Filters - Improved mobile layout */}
...
//                     placeholder="Search members..."
//                     value={state.search}
//                     className="w-full bg-gray-50/80 backdrop-blur-sm border border-gray-200 text-gray-800 rounded-lg sm:rounded-xl py-2 sm:py-3 pl-8 sm:pl-12 pr-4 placeholder-gray-500 focus:outline-none focus:border-teal-500 focus:bg-white/90 transition-all duration-200 text-sm sm:text-base"
//                     onChange={handleSearch}
//                   />
//                   <Search className="absolute left-2 sm:left-4 top-1/2 transform -translate-y-1/2 text-gray-400 w-3 h-3 sm:w-4 sm:h-4" />
//                 </div>
//               </div>
//             </div>
...
//             <button
//               onClick={() => setShowFilters(!showFilters)}
//               className="flex items-center justify-center gap-2 px-3 sm:px-4 py-2 sm:py-3 bg-teal-50 text-teal-700 rounded-lg sm:rounded-xl hover:bg-teal-100 transition-colors duration-200 border border-teal-200 self-start text-sm sm:text-base"
//             >
...
//         {/* Filters Panel - Fully responsive */}
//         {showFilters && (
//           <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-gray-50/80 rounded-lg sm:rounded-xl border border-gray-200">
//             {/* Mobile Search Bar */}
...
//                   placeholder="Search members..."
//                   value={state.search}
//                   className="w-full bg-white border border-gray-300 rounded-lg py-2.5 sm:py-3 pl-8 sm:pl-10 pr-4 text-sm focus:outline-none focus:border-teal-500"
//                   onChange={handleSearch}
//                 />
...
//                   value={state.sortBy}
//                   onChange={(e) => setState(prev => ({ ...prev, sortBy: e.target.value }))}
//                   className="w-full bg-white border border-gray-300 rounded-lg px-2 sm:px-3 py-2 text-xs sm:text-sm focus:outline-none focus:border-teal-500"
//                 >
//                   <option value="name">Name (A-Z)</option>
//                   <option value="referrals">Referrals (High to Low)</option>
//                   <option value="date">Join Date (Newest)</option>
...
//               </div>
//               <div>
//                 <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-2">Per Page</label>
//                 <select
//                   value={state.perPage}
//                   onChange={(e) => {
//                     setState(prev => ({
...
//                     currentPage: 1 
//                   }))}
//                   className="w-full px-3 sm:px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors duration-200 text-xs sm:text-sm"
//                 >
//                   Clear Filters
//                 </button>
//               </div>
...
//         {/* Mobile Cards - Enhanced responsive design */}
//         <div className="grid gap-3 sm:gap-4 lg:hidden">
//           {isLoading ? (
//             [...Array(5)].map((_, i) => (
//               <div key={i} className="bg-white rounded-lg sm:rounded-xl p-3 sm:p-4 border border-gray-200 animate-pulse">
//                 <div className="flex items-center gap-3 mb-3">
...
//           ) : paginatedData.length === 0 ? (
//             <div className="text-center py-12 sm:py-16">
//               <div className="w-16 h-16 sm:w-20 sm:h-20 lg:w-24 lg:h-24 mx-auto mb-3 sm:mb-4 rounded-full bg-gray-100 flex items-center justify-center">
...
//           ) : (
//             paginatedData.map((data, i) => (
//               <div key={i} className="bg-gradient-to-r from-white to-teal-50/50 rounded-lg sm:rounded-2xl p-3 sm:p-4 lg:p-5 border border-teal-100 hover:border-teal-200 hover:shadow-lg transition-all duration-300 group">
//                 {/* Header Section */}
//                 <div className="flex items-start gap-2.5 sm:gap-3 mb-3 sm:mb-4">
//                   <div className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-full bg-gradient-to-br from-teal-500 to-teal-600 flex items-center justify-center text-white font-semibold text-xs sm:text-sm group-hover:scale-110 transition-transform duration-300 flex-shrink-0">
//                     {getAvatarInitials(data.name)}
//                   </div>
                  
//                   <div className="flex-1 min-w-0">
//                     <h3 className="font-semibold text-gray-800 text-sm sm:text-base lg:text-lg mb-0.5 sm:mb-1 break-words line-clamp-2">
//                       {data.name || "N/A"}
//                     </h3>
//                     <p className="text-teal-600 text-xs sm:text-sm break-words line-clamp-1">
//                       {data.username || "N/A"}
//                     </p>
//                   </div>
                  
//                   <span className={`px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full text-xs font-medium whitespace-nowrap ${
//                     data.isActive
//                       ? "bg-green-100 text-green-700 border border-green-200"
//                       : "bg-red-100 text-red-700 border border-red-200"
//                   }`}>
...
//                     <span className="text-gray-600 text-xs sm:text-sm break-all line-clamp-2">
//                       {data.email || "N/A"}
//                     </span>
//                   </div>
...
//                   <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-4">
//                     <div className="flex items-center gap-2">
//                       <Award className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-400 flex-shrink-0" />
...
//         {/* Desktop Table - Enhanced responsive behavior */}
//         <div className="hidden lg:block overflow-x-auto rounded-lg xl:rounded-xl border border-gray-200">
//           <table className="w-full text-sm xl:text-base">
//             <thead>
//               <tr className="bg-gradient-to-r from-teal-600 to-teal-700 text-white text-left">
//                 <th className="text-left py-3 xl:py-4 px-3 xl:px-4 font-semibold rounded-tl-lg xl:rounded-tl-xl">S.No</th>
//                 <th className="text-left py-3 xl:py-4 px-3 xl:px-4 font-semibold">Name</th>
//                 <th className="text-left py-3 xl:py-4 px-3 xl:px-4 font-semibold">Email</th>
//                 <th className="text-left py-3 xl:py-4 px-3 xl:px-4 font-semibold">User Name</th>
//                 <th className="text-left py-3 xl:py-4 px-3 xl:px-4 font-semibold">No of Referral</th>
//                 <th className="text-left py-3 xl:py-4 px-3 xl:px-4 font-semibold">Join Date</th>
//                 <th className="text-left py-3 xl:py-4 px-3 xl:px-4 font-semibold rounded-tr-lg xl:rounded-tr-xl">Status</th>
//               </tr>
//             </thead>
//             <tbody className="bg-white">
//               {isLoading ? (
//                 [...Array(state.perPage)].map((_, i) => (
//                   <tr key={i} className="border-b border-gray-100">
//                     <td className="py-3 xl:py-4 px-3 xl:px-4">.......</td>
//                     <td className="py-3 xl:py-4 px-3 xl:px-4">....... </td>
//                     <td className="py-3 xl:py-4 px-3 xl:px-4">....... </td>
//                     <td className="py-3 xl:py-4 px-3 xl:px-4">....... </td>
//                     <td className="py-3 xl:py-4 px-3 xl:px-4">....... </td>
...
//               ) : paginatedData.length === 0 ? (
//                 <tr>
//                   <td colSpan="7" className="text-center py-12 xl:py-16">
//                     <div className="flex flex-col items-center">
//                       <div className="w-20 h-20 xl:w-24 xl:h-24 mx-auto mb-4 rounded-full bg-gray-100 flex items-center justify-center">
//                         <Users className="w-10 h-10 xl:w-12 xl:h-12 text-gray-400" />
//                       </div>
//                       <h3 className="text-xl font-semibold text-gray-800 mb-2">No members found</h3>
//                       <p className="text-gray-600">
//                         {state.search || state.filterStatus !== 'all' 
...
//                     className="border-b border-gray-100 hover:bg-teal-50/50 transition-colors duration-200 group"
//                   >
//                     <td className="py-3 xl:py-4 px-3 xl:px-4 text-gray-700 font-semibold">
//                       {startIndex + i + 1}
//                     </td>
//                     <td className="py-3 xl:py-4 px-3 xl:px-4">
//                       <div className="flex items-center gap-3">
//                         <div className="font-medium text-gray-800 truncate font-semibold max-w-[150px] xl:max-w-[200px]" title={data.name}>
//                           {data.name || "N/A"}
//                         </div>
//                       </div>
//                     </td>
//                     <td className="py-3 xl:py-4 px-3 xl:px-4 text-gray-700">
//                       <div className="truncate max-w-[120px] xl:max-w-[180px] font-semibold" title={data.email}>
//                         {data.email || "N/A"}
//                       </div>
//                     </td>
//                     <td className="py-3 xl:py-4 px-3 xl:px-4 text-gray-700 font-semibold">
//                       <div className="truncate max-w-[100px] xl:max-w-[150px]" title={data.username}>
//                         {data.username || "N/A"}
//                       </div>
//                     </td>
//                     <td className="py-3 xl:py-4 px-3 xl:px-4 text-gray-700 font-semibold">
//                       {data.totalChainReferrals || 0}
//                     </td>
//                     <td className="py-3 xl:py-4 px-3 xl:px-4 text-gray-700 font-semibold text-xs xl:text-sm">
//                       {data.createdAt ? data.createdAt.slice(0, 10).split("-").reverse().join("-") : "N/A"}
//                     </td>
//                     <td className="py-3 xl:py-4 px-3 xl:px-4">
//                       <span
//                         className={`px-2.5 xl:px-3 py-1 rounded-full text-xs font-bold ${
//                           data.isActive
//                             ? "bg-green-100 text-green-700 border border-green-200"
//                             : "bg-red-100 text-red-700 border border-red-200"
//                         }`}
...
//         {/* Pagination - Fixed to always show when needed */}
//         {totalFilteredUsers > 0 && totalPages > 1 && (
//           <div className="flex flex-col sm:flex-row justify-between items-center mt-4 sm:mt-6 gap-3 sm:gap-4">
//             {/* Results Info */}
//             <div className="text-xs sm:text-sm text-gray-600 order-2 sm:order-1">
//               Showing {startIndex + 1} to {Math.min(startIndex + state.perPage, totalFilteredUsers)} of {totalFilteredUsers} results
//             </div>
...
//         {/* Alternative pagination for small screens */}
//         {totalFilteredUsers > 0 && totalPages > 1 && (
//           <div className="flex sm:hidden justify-center items-center mt-4 gap-2">
//             <button
