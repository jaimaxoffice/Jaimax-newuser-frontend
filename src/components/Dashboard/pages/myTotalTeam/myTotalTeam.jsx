...
//                 <div
//                   key={i}
//                   className="bg-white rounded-md p-3 border border-gray-200 animate-pulse"
//                 >
...
//                         <span
//                           className={`px-2 py-0.5 rounded-md text-xs font-medium flex-shrink-0 ${data.isActive
...
const columns = [
    {
      header: "#",
      render: (_, index) => (state.currentPage - 1) * state.perPage + index + 1,
    },
    {
      header: "Name",
      accessor: "name",
    ...
    {
      header: "Referrals",
      render: (row) => (row.totalDirectReferrals || 0) + (row.totalChainReferrals || 0),
    },
    {
      header: "Join Date",
      render: (row) =>
...
<ReusableTable columns={columns} data={filteredData} isLoading={isLoading}/>
