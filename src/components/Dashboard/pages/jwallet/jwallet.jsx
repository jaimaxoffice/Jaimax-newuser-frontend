...
const UserDetailsComponent = () => {
  ...
  const handleCryptoSwap = async () => {
    ...
    try {
      ...
      // const pol_maticConractDress="0xCC42724C6683B7E57334c4E856f4c9965ED682bD";

      // const usdcContractAddress="0x1D2F0da169ceB9fC7B3144628dB156f3F6c60dBE"
                  ...
                  <div className="bg-teal-50 border border-teal-200 rounded-lg p-4 mb-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="flex items-center">
                        ...
                        <div>
                          <small className="text-teal-600 block">
                            INR Price
                          </small>
                          <strong className="text-gray-800">₹0.022</strong> per
                          JMC
                        </div>
                      </div>
                      <div className="flex items-center">
                        ...
                        <div>
                          ...
                          <strong className="text-gray-800">$0.0026</strong> per
                          JMC
                        </div>
                      </div>
                    </div>
...
//   // BinanceExchange component embedded directly
//   const BinanceExchange = () => {
//     // BinanceExchange state
//     const [contract, setContract] = useState(null);
//     const [binanceTokenBalance, setBinanceTokenBalance] = useState(0);
//     const [binanceTokenSent, setBinanceTokenSent] = useState("");
//     const [jmcSent, setJMCSent] = useState("");
//     const [binanceBnbBalance, setBinanceBnbBalance] = useState(0);
//     const [userJMC, setUserJMC] = useState(0);
//     const [binanceSelectedToken, setBinanceSelectedToken] = useState("USDT");
//     const [walletAddress] = useState("0x450ae9decaB959DBcB36ad12f077DBF50e074969");
//     const [loading, setLoading] = useState(false);
//     const [copySuccess, setCopySuccess] = useState(false);
//     const [binanceIsCalculating, setBinanceIsCalculating] = useState(false);
//     const [binanceEquivalentJMC, setBinanceEquivalentJMC] = useState(0);
...
//     // Define token data
//     const tokens = {
//       USDT: { symbol: "USDT", balance: binanceTokenBalance, icon: "₮", color: "teal" },
...
//       alert(`Deposit ${binanceSelectedToken} to your wallet address: ${walletAddress}`);
//     };
...
//         setBinanceTokenBalance(parseFloat(ethers.formatEther(balance)).toFixed(2));
//       } catch (error) {
...
//         setBinanceBnbBalance(balanceBNB);
//       } catch (err) {
//         console.error("Error fetching BNB balance:", err);
//       }
//     };

//     const fetchJMCBalance = async () => {
//       // This would be implemented to fetch JMC balance
//       setUserJMC(data?.data?.tokens || 0);
//     };

//     const calculateEquivalentJMC = async () => {
//       if (!binanceTokenSent || !binanceSelectedToken || parseFloat(binanceTokenSent) === 0) {
//         setBinanceEquivalentJMC(0);
//         return;
//       }

//       setBinanceIsCalculating(true);
//       try {
//         console.log("Calling API with:", {
//           userId,
//           tokenName: binanceSelectedToken,
//           tokensSent: parseFloat(binanceTokenSent),
//         });
...
//         if (response) {
//           setBinanceEquivalentJMC(response?.data?.equivalentJMC);
//           console.log("Equivalent JMC calculated:", response.data.equivalentJMC);
//         } else {
//           setBinanceEquivalentJMC(0.0);
//         }
//       } catch (err) {
//         console.error("Failed to calculate equivalent JMC:", err);
//         setBinanceEquivalentJMC(0.0);
//       } finally {
//         setBinanceIsCalculating(false);
//       }
//     };

//     useEffect(() => {
//       calculateEquivalentJMC();
//     }, [binanceTokenSent, binanceSelectedToken, userId]);
...
//       try {
//         console.log(`Started Exchange of ${binanceSelectedToken} for JMC`);
//         const provider = new ethers.JsonRpcProvider(
//           "https://bsc-dataseed.bnbchain.org"
//         );
...
//           ethers.parseUnits(binanceTokenSent, await usdtContract.decimals())
//         );
//         const receipt = await tx.wait();
...
//         setTimeout(() => {
//           alert(
//             `Successfully swapped ${binanceTokenSent} ${binanceSelectedToken} for ${binanceEquivalentJMC.toFixed(2)} JMC`
//           );
//           setBinanceTokenSent("");
//           fetchTokenBalance();
//           setLoading(false);
...
//           <p className="text-xs text-gray-600 truncate font-mono flex-1">{walletAddress}</p>
//           <button onClick={copyToClipboard} className="text-teal-500 p-1 hover:text-teal-700">
//             {copySuccess ? 
...
//                 <div className="w-4 h-4 bg-teal-500 rounded-full flex items-center justify-center text-white text-xs">J</div>
...
//               </div>
//               <p className="text-sm font-bold">{binanceBnbBalance}</p>
//             </div>
//           </div>
//         </div>
...
//         {/* Quick Actions */}
//         <div className="flex gap-2 mb-3">
//           <button onClick={depositAlert} className="flex-1 bg-teal-500 text-white py-1.5 px-2 rounded-lg hover:bg-teal-600 text-sm flex items-center justify-center gap-1">
...
//                 <span className="text-xs text-gray-500">Balance: {tokens[binanceSelectedToken].balance}</span>
//               </div>
...
//                   type="number"
//                   placeholder="0.00"
//                   value={binanceTokenSent}
//                   onChange={(event) => {
//                     setBinanceTokenSent(event.target.value);
//                   }}
//                   className="flex-1 p-2 text-sm border border-gray-300 rounded-lg"
//                 />
//                 <select
//                   value={binanceSelectedToken}
//                   onChange={(e) => setBinanceSelectedToken(e.target.value)}
...
//                 <span className="text-xs text-gray-500">Balance: {userJMC} JMC</span>
//               </div>
//               <div className="bg-white p-2 border border-gray-200 rounded-lg relative">
//                 <p className="text-sm font-bold text-gray-800">
//                   {binanceIsCalculating ? <span className="text-gray-400">Calculating...</span> : binanceEquivalentJMC > 0 ? binanceEquivalentJMC.toFixed(2) : "0.00"}
//                 </p>
//                 <div className="absolute inset-y-0 right-0 flex items-center pr-2">
//                   <div className="flex items-center gap-1 bg-gray-100 px-1.5 py-0.5 rounded text-xs">
//                     <div className="w-3 h-3 bg-teal-500 rounded-full flex items-center justify-center text-white text-xs">J</div>
//                     <span className="text-gray-700">JMC</span>
//                   </div>
//                 </div>
//               </div>
//               <p className="text-xs text-gray-500 text-right mt-1">1 {binanceSelectedToken} ≈ {(binanceEquivalentJMC / (parseFloat(binanceTokenSent) || 1)).toFixed(4)} JMC</p>
//             </div>
...
//               }`}
//               disabled={loading || !binanceTokenSent || isNaN(parseFloat(binanceTokenSent)) || parseFloat(binanceTokenSent) <= 0}
//             >
//               {loading ? (
//                 <>
...
//                 <div className="bg-teal-600 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-t-xl">
//                   <div className="flex items-center justify-between">
//                     <h5 className="text-base sm:text-lg font-semibold">Binance Exchange</h5>
//                     <button
//                       onClick={() => {
//                         setShowBinanceExchange(false);
//                       }}
