...
// import { Eye, EyeOff, Copy, Shield, Lock, AlertTriangle, Clock, CheckCircle } from 'lucide-react';
...
//         try {
//             const response = await fetch("http://localhost:3002/api/reveal/2fa-status", {
...
//         try {
//             const response = await fetch("http://localhost:3002/api/reveal/start-2fa", {
//                 method: "POST",
//                 headers: { Authorization: `Bearer ${token}` }
//             });
...
//         try {
//             const response = await fetch("http://localhost:3002/api/reveal/verify-2fa", {
//                 method: "POST",
//                 headers: {
...
//             const { encrypted, iv } = await encryptPayload(payload, password);

//             const response = await fetch('http://localhost:3002/api/reveal/reveal', {
//                 method: 'POST',
//                 headers: {
//                     'Content-Type': 'application/json',
//                     'Authorization': `Bearer ${token}`
//                 },
//                 body: JSON.stringify({ encrypted, iv, password, otp: otp || null })
//             });

//             const res = await response.json();

//             if (res.success === 0) {
//                 throw new Error(res.message || 'Failed to reveal data');
//             }

//             // backend returns encrypted + iv inside res.data
...
//     const handleCopy = () => {
//         const textToCopy = revealType === 'seed'
...
//                             <h1 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-800">Access Sensitive Data</h1>
//                         </div>
...
//                                         {/* <li>Check for cameras or shoulder surfers</li> */}
...
//                                     disabled={loading}
//                                     className="w-full bg-gradient-to-r from-teal-500 to-teal-600 hover:from-teal-600 hover:to-teal-700 disabled:from-gray-400 disabled:to-gray-400 text-white font-semibold py-2.5 sm:py-3 rounded-lg transition-all text-sm sm:text-base"
//                                 >
//                                     {loading ? 'Processing...' : 'Enable Google Authenticator'}
//                                 </button>
//                             </div>
//                         )}

//                         {qrCode && (
//                             <div className="mb-4 sm:mb-6 bg-gray-50 rounded-lg p-3 sm:p-4 border border-teal-200">
//                                 <p className="text-gray-800 mb-3 text-center font-semibold text-sm sm:text-base">
//                                     Scan this QR code in Google Authenticator
//                                 </p>
//                                 <img src={qrCode} alt="2FA QR code" className="mx-auto w-36 h-36 sm:w-48 sm:h-48 rounded-lg" />
//                                 {error && (
//                                     <p className="text-yellow-700 text-xs sm:text-sm mt-3 text-center">{error}</p>
//                                 )}
...
//                                         className="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-gray-800 focus:outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-200 text-center"
//                                         maxLength={6}
//                                     />
//                                     <button
//                                         onClick={verify2FA}
//                                         disabled={loading || otp.length !== 6}
//                                         className="bg-teal-500 hover:bg-teal-600 text-white font-semibold px-4 rounded-lg"
//                                     >
//                                         Verify
//                                     </button>
//                                 </div>
//                                 <button
//                                     onClick={() => setQrCode('')}
//                                     className="mt-3 w-full text-xs sm:text-sm text-gray-600 hover:text-gray-800"
//                                 >
//                                     Close
//                                 </button>
//                             </div>
...
//                                     className={`p-3 sm:p-4 rounded-lg border-2 transition-all ${revealType === 'seed'
...
//                                     className={`p-3 sm:p-4 rounded-lg border-2 transition-all ${revealType === 'private'
//                                         ? 'border-teal-500 bg-teal-50'
//                                         : 'border-gray-300 bg-gray-50'
//                                         }`}
//                                 >
...
//                             className="w-full bg-gradient-to-r from-teal-500 to-teal-600 hover:from-teal-600 hover:to-teal-700 text-white font-semibold py-2.5 sm:py-3 rounded-lg transition-all text-sm sm:text-base"
//                         >
...
//                             <h1 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-800">Verify Your Identity</h1>
//                         </div>

//                         {error && typeof error === 'string' && !error.includes('already enabled') && (
...
//                                     🔒 2FA is enabled. Please enter your authenticator code below.
//                                 </p>
//                             </div>
//                         )}
...
//                                     className="w-full bg-white border border-gray-300 rounded-lg px-3 sm:px-4 py-2 sm:py-3 text-gray-800 focus:outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-200 text-sm sm:text-base"
...
//                                         }
//                                         className="w-full bg-white border border-gray-300 rounded-lg px-3 sm:px-4 py-2 sm:py-3 text-gray-800 focus:outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-200 text-center text-xl sm:text-2xl tracking-widest"
//                                         placeholder="000000"
//                                         maxLength={6}
//                                     />
...
//                                     className="w-full sm:flex-1 bg-gray-200 hover:bg-gray-300 text-gray-800 font-semibold py-2.5 sm:py-3 rounded-lg transition-colors text-sm sm:text-base"
//                                 >
//                                     Cancel
//                                 </button>
//                                 <button
//                                     onClick={handleReveal}
//                                     disabled={loading || !password || (is2FAEnabled && otp.length !== 6)}
//                                     className="w-full sm:flex-1 bg-gradient-to-r from-teal-500 to-teal-600 hover:from-teal-600 hover:to-teal-700 disabled:from-gray-400 disabled:to-gray-400 disabled:cursor-not-allowed text-white font-semibold py-2.5 sm:py-3 rounded-lg transition-all text-sm sm:text-base"
//                                 >
...
//                                 onClick={handleCopy}
//                                 disabled={!revealed}
//                                 className="w-full sm:flex-1 bg-gradient-to-r from-teal-500 to-teal-600 hover:from-teal-600 hover:to-teal-700 disabled:from-gray-400 disabled:to-gray-400 disabled:cursor-not-allowed text-white font-semibold py-2.5 sm:py-3 rounded-lg transition-all flex items-center justify-center gap-2 text-sm sm:text-base"
//                             >
//                                 {copied ? (
//                                     <>
//                                         <CheckCircle size={18} />
...
//                             <button
//                                 onClick={handleClose}
//                                 className="w-full sm:w-auto px-6 bg-gray-200 hover:bg-gray-300 text-gray-800 font-semibold py-2.5 sm:py-3 rounded-lg transition-colors text-sm sm:text-base"
//                             >
//                                 Close
//                             </button>
//                         </div>
...
const SecureRevealComponent = () => {
    ...
    // RTK Query hooks
    const { data: statusData, isLoading: checking2FA, refetch: refetch2FA } = useCheck2FAStatusQuery();
    ...
    const [revealSensitiveData, { isLoading: loadingReveal }] = useRevealSensitiveDataMutation();
    ...
    const handleReveal = async () => {
        ...
        if (is2FAEnabled && otp.length !== 6) return setError('Please enter a valid 6-digit 2FA code');
                        ...
                        {is2FAEnabled ? (
                            ...
                            <div className="mb-4 sm:mb-6">
                                ...
                                <button
                                    ...
                                    className="w-full bg-gradient-to-r from-teal-500 to-teal-600 hover:from-teal-600 hover:to-teal-700 disabled:from-gray-400 disabled:to-gray-400 text-white font-semibold py-2.5 sm:py-3 rounded-lg transition-all text-sm sm:text-base"
                                >
                                    {loadingSetup ? 'Processing...' : 'Enable Google Authenticator'}
                                </button>
                            </div>
                        )}
                                ...
                                <div className="bg-white w-full max-w-sm rounded-2xl p-5 shadow-xl relative">
                                    ...
                                    <div className="flex gap-2 mt-4">
                                        ...
                                        <button
                                            ...
                                            className="flex-1 bg-teal-500 hover:bg-teal-600 disabled:bg-gray-400 text-white font-semibold py-2 rounded-lg"
                                        >
                            ...
                            <div className="mt-3 bg-red-50 border border-red-200 rounded-lg p-3">
                                ...
                                <div className="flex flex-col sm:flex-row gap-1 sm:gap-3">
                                    <input
                                        ...
                                        className="
      w-full sm:flex-1
      border border-gray-300 rounded-md sm:rounded-lg
      px-2 py-1.5 sm:px-4 sm:py-2.5
      text-sm sm:text-lg
      tracking-widest
      text-center
      focus:outline-none
      focus:ring-1 sm:focus:ring-2 focus:ring-red-400
    "
                                    />
