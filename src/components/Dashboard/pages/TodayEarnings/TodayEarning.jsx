...
import React, { useState, useEffect } from 'react';
import { Crown, Trophy, Zap, Clock, Medal, Star, Percent, Check, ChevronRight, Sparkles, Target, TrendingUp, Gift, AlertCircle, X } from 'lucide-react';
import { useGetTodayEarningStatusMutation } from './userEarningApiSlice';
import Loader from "../../../Loader/loader";
import ReferralModal from '../../modals/referalModal';
...
const VictoryLeaderboard = () => {
  ...
  const ErrorAlert = ({ error, onClose }) => (
    <div className="fixed top-4 left-4 right-4 z-50 animate-in slide-in-from-top-full duration-300">
      <div className="bg-red-500/95 backdrop-blur-sm border-2 border-red-400 text-white px-4 py-3 rounded-lg shadow-2xl max-w-md mx-auto">
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            <AlertCircle size={20} className="mr-2 text-red-200" />
            <div>
              <div className="font-semibold text-sm">Error Loading Data</div>
          ...
          <button
            onClick={onClose}
            className="ml-3 p-1 hover:bg-red-400/50 rounded-full transition-colors duration-200"
          >
  ...
  return (
    <>
      {/* Error Alert - Fixed Position at Top */}
      {error && showError && (
        <ErrorAlert 
          error={error} 
