  ...
  const columns = [
    {
      header: "S.No",
      render: (row, index) =>
        queryState.currentPage * queryState.perPage -
        (queryState.perPage - 1) +
        index,
    },
    ...
    {
      header: "Referrals",
      render: (row) => row.totalDirectReferrals + row.totalChainReferrals,
    },
    {
      header: "Join Date",
      render: (row) =>
    ...
    },
    {
      ...
      render: (row) => (
        <span
          className={`text-xs px-2 py-1 rounded-full text-white ${
            row.isActive ? "bg-teal-600" : "bg-teal-400"
          }`}
        >
