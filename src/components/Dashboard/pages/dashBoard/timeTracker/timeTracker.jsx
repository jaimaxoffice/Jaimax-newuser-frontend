...
//       status: isActive ? "Live" : round.status === 1 ? "Sold" : "Upcoming",
//       statusColor: isActive
//         ? "bg-emerald-500"
//         : round.status === 2
//         ? "bg-red-500"
//         : "bg-amber-500",
//       type: isActive ? "active" : round.status === 1 ? "sold" : "upcoming",
//       prices: {
...
const SlabTabs = () => {
  ...
  // API hooks
  const { data: apiData, refetch: refetchRounds, isLoading: isRoundsLoading } = useGetRoundQuery();
  const { data: userData, refetch: refetchUserData } = useUserDataQuery();
  const { data: settings } = useGetAdminSettingsQuery();
  const [addOrder] = useAddOrderMutation();
  const [proceedOrder] = useProceedOrderMutation();
  ...
  // Set active slab as the initial tab when data is loaded
  useEffect(() => {
    if (slabsData.length > 0 && initialLoadRef.current) {
      // Find the index of the first active slab
      const activeSlabIndex = slabsData.findIndex(slab => slab.type === "active");
      
      // If an active slab is found, set it as the active tab
      if (activeSlabIndex !== -1) {
        setActiveTab(activeSlabIndex);
      }
  ...
  // Loading state - only show if no data at all
  if (isRoundsLoading || slabsData.length === 0) {
    return (
      <div className="bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 rounded-2xl w-full max-w-7xl mx-auto shadow-2xl border border-white/50 backdrop-blur-sm">
        <div className="px-4 py-8 text-center">
