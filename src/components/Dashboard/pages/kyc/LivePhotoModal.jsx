...
const LivePhotoModal = ({
  ...
  const handleCaptureWithCountdown = () => setCountdown(3);
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto">
      ...
      <div className="flex min-h-full items-center justify-center p-3 sm:p-4">
        <div className="relative w-full max-w-sm sm:max-w-md transform overflow-hidden rounded-2xl sm:rounded-3xl bg-white shadow-2xl">
          ...
          <div className="p-4 sm:p-5">
            ...
            {/* User name badge */}
            {userName && (
              <div className="mb-3 bg-teal-50 rounded-xl px-3 py-2 text-center">
                <p className="text-teal-700 text-xs sm:text-sm">
                  <span className="font-medium">For:</span> {userName}
                </p>
              </div>
            )}

            {/* Camera Container */}
            <div className="relative aspect-[4/3] bg-gray-900 rounded-xl sm:rounded-2xl overflow-hidden mb-4">
              ...
              {!capturedPhoto && (
                <>
                  ...
                  {cameraActive && (
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                      <div className="w-28 sm:w-40 h-36 sm:h-52 border-[3px] border-white/40 border-dashed rounded-[50%] relative">
                        {/* Instruction badge */}
                        <div className="absolute -bottom-8 left-1/2 transform -translate-x-1/2 w-max">
                          <div className="bg-black/60 backdrop-blur-sm px-3 py-1.5 rounded-full">
                            <p className="text-white text-[10px] sm:text-xs font-medium">{instruction}</p>
                          </div>
                        </div>
                        
                        {/* Corner markers */}
                        <div className="absolute -top-1 -left-1 w-4 h-4 border-t-2 border-l-2 border-teal-400 rounded-tl-lg" />
                        <div className="absolute -top-1 -right-1 w-4 h-4 border-t-2 border-r-2 border-teal-400 rounded-tr-lg" />
                        <div className="absolute -bottom-1 -left-1 w-4 h-4 border-b-2 border-l-2 border-teal-400 rounded-bl-lg" />
                        <div className="absolute -bottom-1 -right-1 w-4 h-4 border-b-2 border-r-2 border-teal-400 rounded-br-lg" />
                      </div>
                    </div>
                  )}
                  ...
                  {countdown !== null && countdown > 0 && (
                    <div className="absolute inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                      <div className="w-20 h-20 sm:w-24 sm:h-24 bg-teal-600 rounded-full flex items-center justify-center">
                        <span className="text-4xl sm:text-5xl font-bold text-white">{countdown}</span>
                      </div>
                    </div>
                  )}
              ...
              {error && (
                <div className="absolute inset-0 flex items-center justify-center bg-gray-900">
                  <div className="text-center p-4">
                    <div className="w-12 h-12 sm:w-14 sm:h-14 bg-red-500/20 rounded-full flex items-center justify-center mx-auto mb-3">
                      <svg className="w-6 h-6 sm:w-7 sm:h-7 text-red-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                      </svg>
                    </div>
                    <p className="text-white text-xs sm:text-sm mb-3">{error}</p>
                    <button
                      onClick={startCamera}
                      className="px-4 py-2 bg-teal-600 text-white rounded-xl hover:bg-teal-700 transition-colors text-xs sm:text-sm font-medium"
                    >
                      Try Again
                    </button>
                  </div>
              ...
              {!cameraActive && !error && !capturedPhoto && (
                <div className="absolute inset-0 flex items-center justify-center bg-gray-900">
                  <div className="text-center">
                    <Spinner className="w-8 h-8 sm:w-10 sm:h-10 text-teal-500 mx-auto mb-3" />
                    <p className="text-white text-xs sm:text-sm">Starting camera...</p>
                  </div>
                </div>
              )}
            </div>
            ...
            {/* Success Message */}
            {capturedPhoto && (
              <div className="mb-4 bg-teal-50 rounded-xl p-3 flex items-start gap-2.5">
                ...
                <div>
                  <p className="text-teal-800 font-medium text-sm">Photo captured!</p>
                  <p className="text-teal-600 text-xs mt-0.5">Review and submit, or retake if needed.</p>
                </div>
              </div>
            )}
                ...
                <>
                  <button
                    ...
                    className="flex-1 py-2.5 sm:py-3 rounded-xl font-medium border-2 border-gray-200 
                             text-gray-600 hover:bg-gray-50 text-xs sm:text-sm transition-all"
                  >
                    Cancel
                  </button>
                  <button
                    onClick={handleCaptureWithCountdown}
                    disabled={!cameraActive || countdown !== null}
                    className={`flex-1 py-2.5 sm:py-3 rounded-xl font-medium text-white text-xs sm:text-sm
