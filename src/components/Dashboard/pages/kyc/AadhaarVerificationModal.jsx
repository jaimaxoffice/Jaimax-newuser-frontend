...
//     if (!aadhaarName || !digilockerName) return false;
//     const normalize = (str) => {
//       return str
//         .toLowerCase()
//         .replace(/[^a-z\s]/g, "")
...
//           const matchResult = matchNames(aadhaarName, digilockerName);

//           setNameMatchStatus(matchResult);

//           if (matchResult.matched) {
//             setStep("success");
//             toast.success("Aadhaar verified and name matched!", {
//               position: "top-center",
//             });
//           } else {
//             setStep("mismatch");
//             toast.warning(
//               `Name similarity: ${matchResult.details.similarityScore}`,
//               {
//                 position: "top-center",
//               }
//             );
...
//                         <span className="font-medium">DigiLocker Name:</span>{" "}
//                         {digilockerName}
//                       </p>
//                     </div>
//                   )}
...
//                   <button
//                     onClick={onSkip}
//                     className="w-full py-3 text-gray-600 hover:text-gray-800 text-sm underline"
//                   >
...
//                     </div>
//                   )}
//                   {digilockerName && nameMatchStatus?.matched && (
//                     <div className="flex justify-between items-center">
...
//                   </h2>
//                   <p className="text-gray-600">
//                     The names don't match sufficiently
//                   </p>
//                 </div>  
...
const AadhaarVerificationModal = ({
  ...
  const matchDob = (aadhaarDob, digilockerDob) => {
    if (!aadhaarDob || !digilockerDob) return { matched: false, reason: "DOB data missing" };

    // Normalize date formats (handle DD-MM-YYYY, DD/MM/YYYY, YYYY-MM-DD etc.)
    const normalizeDate = (dateStr) => {
      if (!dateStr) return null;
      
      // Convert string to standardized format
    ...
    const normalizedAadhaarDob = normalizeDate(aadhaarDob);
    const normalizedDigilockerDob = normalizeDate(digilockerDob);

    console.log("[DOB Match] Aadhaar DOB:", normalizedAadhaarDob);
    console.log("[DOB Match] DigiLocker DOB:", normalizedDigilockerDob);

    const matched = normalizedAadhaarDob === normalizedDigilockerDob;
    ...
    try {
      ...
      if (response?.success || response?.status_code === 200) {
        ...
        if (digilockerDob && aadhaarDob) {
          ...
          if (!dobResult.matched) {
            // DOB doesn't match - show DOB mismatch screen
            setStep("dobMismatch");
            toast.error("Date of birth mismatch detected!", {
              position: "top-center",
            });
            return;
          }
        ...
        // SECOND: If DOB matches (or no DigiLocker DOB to compare), check name
        if (digilockerName && aadhaarName) {
