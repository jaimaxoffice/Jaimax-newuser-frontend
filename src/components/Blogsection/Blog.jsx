...
const convertSectionsToHTML = (sections) => {
  ...
  sections.forEach((section) => {
    switch (section.type) {
      case "heading":
        html += `<h3 class="text-xl font-bold mt-6 mb-3 text-cyan-100">${section.content}</h3>`;
        break;
      case "paragraph":
        html += `<p class="mb-4 text-gray-300">${section.content}</p>`;
        break;
      case "unordered_list":
        html +=
...
const BlogLayout = () => {
  ...
  const sharePost = (platform) => {
    ...
    const title = selectedPost?.headline || "Check out this article";
  ...
  // Show individual post view if selected
  if (selectedPost) {
    return (
      <div
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div className="lg:col-span-3">
              <div className="relative group">
                <div className="absolute -inset-0.5 bg-gradient-to-r from-cyan-400 to-teal-400 rounded-2xl blur opacity-20" />
                <div className="relative bg-gradient-to-br from-cyan-950/80 to-teal-950/70 backdrop-blur-xl rounded-2xl border border-cyan-400/30 p-8">
                  {/* Post Header */}
                  <div className="mb-6">
                    ...
                    {selectedPost.content?.title && (
                      <h2 className="text-xl lg:text-2xl font-semibold text-cyan-300 mb-4">
                        {selectedPost.content.title}
                      </h2>
                    )}
                  </div>
                  ...
                  <div className="mt-8 pt-6 border-t border-cyan-700/30">
                    <h4 className="text-cyan-100 font-medium mb-4">
                      Share this article:
                    </h4>
                ...
                <div className="relative bg-gradient-to-br from-cyan-950/80 to-teal-950/70 backdrop-blur-xl rounded-2xl border border-cyan-400/30 p-5">
                  ...
                  <div className="space-y-3">
                    {allPosts.slice(0, 5).map((post) => (
                      <div
                        key={post.id}
                        className="cursor-pointer hover:bg-cyan-500/10 p-2 rounded-lg transition-colors"
                        onClick={() => handleCardClick(post)}
                      >
                        <h4 className="text-sm text-cyan-200 line-clamp-2">
                          {post.headline}
                        </h4>
          ...
          <main className="lg:col-span-3">
            ...
            {postsLoading && filteredPosts.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-cyan-300/60 mt-4">Loading articles...</p>
              </div>
                  ...
                  >
                    ...
                    <div className="relative h-full p-2 bg-gradient-to-br from-slate-900/95 to-gray-900/90 backdrop-blur-xl rounded-2xl border border-cyan-400/20 group-hover:border-cyan-400/40 overflow-hidden transition-all duration-500 flex flex-col">
                      ...
                      <div className="p-5 flex flex-col flex-grow">
                        ...
                        <div
                          className="text-sm text-gray-300 group-hover:text-cyan-100/90 line-clamp-3 mb-4 flex-grow"
                          dangerouslySetInnerHTML={{
                            __html: DOMPurify.sanitize(
