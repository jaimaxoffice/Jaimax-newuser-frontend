...
// Avatar with Dropdown component - reusable for both normal and error states
const AvatarWithDropdown = React.memo(({ 
  profileImage, 
  isError = false,
  isDropdownOpen,
  onAvatarClick,
  ...
  <div className="relative group" ref={dropdownRef}>
    ...
    <div
      className={`w-10 h-10 rounded-full overflow-hidden border-2 cursor-pointer ${
        isError ? 'border-red-300' : 'border-teal-100'
      }`}
    ...
    >
      {profileImage && !isError ? (
        ...
        <div className={`w-full h-full flex items-center justify-center ${
          isError ? 'bg-red-100' : 'bg-teal-50'
        }`}>
          <FaUser className={`text-xl ${isError ? 'text-red-600' : 'text-teal-600'}`} />
        </div>
      )}
    </div>
...
ProfileIcon.displayName = "ProfileIcon";
LogoutIcon.displayName = "LogoutIcon";
AvatarWithDropdown.displayName = "AvatarWithDropdown";
...
const Header = React.memo(() => {
  ...
  // Move state and refs BEFORE any conditional returns
  const [isDropdownOpen, setIsDropdownOpen] = useState(false);
  const dropdownRef = useRef(null);
  ...
  // Error state component - NOW WITH DROPDOWN
  if (error) {
    return (
      <header className="relative p-4 rounded-xl text-white border-r border-teal-600/20 bg-gradient-to-br from-[#085358] via-[#085358] to-[#085358] sm:px-6 lg:px-8">
        <h1 className="invisible md:visible text-xl text-start sm:text-2xl md:text-3xl font-extrabold">
        ...
        <div className="absolute top-3 right-2 sm:right-3 flex items-center space-x-2">
          ...
          {/* Avatar with Dropdown for Error State */}
          <AvatarWithDropdown
            profileImage={null}
            isError={true}
  ...
  return (
    <header className="relative p-4 rounded-xl text-white border-r border-teal-600/20 bg-gradient-to-br from-[#085358] via-[#085358] to-[#085358] sm:px-6 lg:px-8">
      ...
      <div className="absolute top-3 right-2 sm:right-3 flex items-center space-x-2">
        ...
        {/* Avatar with Dropdown for Normal State */}
        <AvatarWithDropdown
