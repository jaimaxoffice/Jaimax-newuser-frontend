    ...
    <div className="country-dropdown-container relative">
      ...
      {isOpen && (
        <div className="fixed top-4 right-4 bg-blue-100 p-2 rounded text-xs z-50">
          <div>Dropdown Open: {isOpen.toString()}</div>
          <div>Countries Count: {filteredCountries.length}</div>
          <div>Data Source Length: {dataSource.length}</div>
        </div>
      )}
    </div>
  );
...
const RegisterComponent = ({ onSubmit, onToggleMode, isVisible,showModal,
  ...
  const handleCheckboxChange = (e) => {
    ...
    if (isChecked) {
      ...
      setIsChecked(false);
      // isConfirmAgree will be handled by AuthContainer
    }
  };
