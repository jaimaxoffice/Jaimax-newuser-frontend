...
const RegisterComponent = ({ onSubmit, onToggleMode, isVisible }) => {
  ...
  const handleSendOTP = async () => {
    // Validate phone specifically before sending OTP
    ...
    try {
      // Pass all formData fields, ensuring phone is formatted
      const response = await resendOtp({
        ...formData, // Spreads all current form data
        phone: selectedCode + phoneValue // Ensures the phone number is correctly formatted
      });
      ...
      if (response.data?.success) {
        ...
        setNotification({
          type: 'success',
          message: `OTP sent to ${selectedCode}${phoneValue}!`
        });
      } else {
        setNotification({
          type: 'error',
  ...
  const handleSubmit = async (e) => {
    ...
    let currentErrors = {};
    Object.keys(formData).forEach(key => {
        // Perform validation and update a temporary errors object
        const tempErrors = { ...currentErrors }; // Copy current errors for mutation
        switch (key) {
            case 'name':
            ...
            case 'phone':
                ...
                else if (!/^\d{7,15}$/.test(formData.phone.replace(/\s/g, ''))) tempErrors.phone = 'Please enter a valid phone number';
            ...
            case 'password':
                ...
                else if (!/[a-z]/.test(formData.password)) tempErrors.password = 'Password must contain at least one lowercase letter';
                else if (!/[A-Z]/.test(formData.password)) tempErrors.password = 'Password must contain at least one uppercase letter';
                else if (!/\d/.test(formData.password)) tempErrors.password = 'Password must contain at least one number';
                else delete tempErrors.password;
                break;
            case 'confirmPassword':
                if (!formData.confirmPassword) tempErrors.confirmPassword = 'Please confirm your password';
                else if (formData.confirmPassword !== formData.password) tempErrors.confirmPassword = 'Passwords must match';
                else delete tempErrors.confirmPassword;
                break;
            case 'referralId':
                if (formData.referralId && !/^[A-Za-z0-9]*$/.test(formData.referralId)) tempErrors.referralId = 'Referral ID can only contain letters and numbers';
                else delete tempErrors.referralId;
                break;
            case 'otp':
                if (otpSent && !formData.otp) tempErrors.otp = 'OTP is required';
                else if (formData.otp && !/^\d{6}$/.test(formData.otp)) tempErrors.otp = 'OTP must be 6 digits';
                else delete tempErrors.otp;
                break;
        }
        currentErrors = tempErrors; // Update currentErrors for the next iteration
    });
    setErrors(currentErrors); // Set the accumulated errors at the end
    ...
    if (!otpSent) {
      setNotification({
        type: 'error',
        message: 'Please send and verify your phone number with OTP first.'
      });
      return;
    }
    ...
    try {
      ...
      console.error("Registration submission error:", error); // Log the error for debugging
      setNotification({
        type: 'error',
    ...
    <div className={`w-full max-w-md transition-all duration-500 transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
      ...
      <div className="space-y-3">
        ...
        <div className="relative mb-5">
          <div className="flex gap-3">
            ...
            <button
              ...
              disabled={isResendLoading || otpSent || !formData.phone || Object.keys(errors).some(key => key === 'phone')}
              className={`px-4 py-3 rounded-lg font-semibold whitespace-nowrap transition-all duration-200 ${
                otpSent
                  ? 'bg-green-100 text-green-700 cursor-default'
                  : isResendLoading
                  ? 'bg-gray-100 text-gray-500 cursor-not-allowed'
                  : formData.phone && /^\d{7,15}$/.test(formData.phone.replace(/\s/g, '')) && !errors.phone
        ...
        <button
          type="submit"
          onClick={handleSubmit}
          disabled={isRegisterLoading || isVerifyLoading || (otpSent && !formData.otp) || Object.keys(errors).length > 0}
